// Ported from Outspoke (github.com/minburg/outspoke, GPLv3) audio/SileroVadFilter.kt.
...
class SileroVadFilter(
    ...
    override fun process(chunk: AudioChunk, rms: Float): List<AudioChunk> {
        ...
        if (e != null && s != null && chunk.samples.size == REQUIRED_SAMPLES) {
            ...
            // Unaligned chunk size or missing session: fall back purely on RMS gating.
    ...
    companion object {
        ...
        /** 1 frame x 30 ms = 30 ms - opens gate on first confirmed speech frame to avoid clipping onset phonemes. */
        private const val ONSET_FRAMES = 1

        /** 20 frames x 30 ms = 600 ms - gives model pre-speech silence as acoustic context. */
        private const val LEAD_IN_FRAMES = 20

        /** 15 frames x 30 ms = 450 ms - captures trailing soft syllables. */
        private const val HANGOVER_FRAMES = 15

        /** 20 frames x 30 ms = 600 ms of silence after hangover before emitting an utterance boundary. */
